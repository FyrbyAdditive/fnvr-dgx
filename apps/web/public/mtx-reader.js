// Vendored from MediaMTX v1.17.x
// (https://github.com/bluenviron/mediamtx — MIT licensed).
// Source: served by MediaMTX itself at /<path>/reader.js.
// We bundle it because (a) MediaMTX may not be on the same origin as
// the web app at every deployment, and (b) the reader handles
// non-advertised codec negotiation (H.265, AV1) and trickle-ICE that
// our previous in-house WHEP client didn't.
//
// Update strategy: re-fetch from a running mediamtx container when
// upgrading mediamtx. No fnvr-specific edits below this line.
'use strict';
...
class MediaMTXWebRTCReader {
  ...
  #setupPeerConnection(iceServers) {
    ...
    this.pc = new RTCPeerConnection({
      ...
      // fnvr edit: max-bundle so all m-sections share one transport.
      // Stock reader defaults to 'balanced'; with the audio
      // transceiver below that produced 3 separate transports
      // (audio, video, data-channel) and DTLS handshake hung on the
      // audio one because our camera publishes are video-only — the
      // overall PC reports "connected" via the data-channel transport
      // but no media frames flow.
      bundlePolicy: 'max-bundle',
    });

    const direction = 'recvonly';
    this.pc.addTransceiver('video', { direction });
    // fnvr edit: skip audio — our camera publishes are all video-only,
    // and a recvonly audio transceiver against a video-only path
    // leaves an unanswered audio m-section that hangs DTLS for the
    // whole bundle. If we ever add audio cameras, gate this on a
    // per-camera flag.
