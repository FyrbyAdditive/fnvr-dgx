...
- **The recording branch is `rtspclientsink → MediaMTX`, not `qtmux + filesink`.** Earlier iterations wrote MP4s directly from the supervisor (first via `splitmuxsink`, then `qtmux + filesink` with `reserved-moov-update-period`). Both paths broke under crashes / hourly rotation in subtle ways. MediaMTX's built-in recorder owns the on-disk fMP4 layout now and the supervisor's recording branch is just a TCP-RTSP push; do not regress to writing MP4s in-process.
